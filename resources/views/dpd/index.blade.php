            ...
            <table id="datatablesSimple" class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <th style="width: 20%" class="text-center">Wilayah</th>
                    ...
                    @foreach ($user as $index => $data)
                        <tr>
                            ...
                            <td>
                                ...
                                <form id="formAkses{{ $data->id }}" action="{{ route('dpd.updateAkses', $data->id) }}" method="POST" class="d-none">
                                    @csrf
                                    @method('PATCH')
                                    <div class="d-flex gap-1">
                            ...
                            <td>
                                <div id="inputReadonlyDaerah{{ $data->id }}" class="d-flex gap-1">
                                    <input readonly type="text" class="form-control form-control-sm w-50" value="{{ ($data->daerah) ? $data->daerah->nama : '' }}">
                                    <button class="btn btn-warning btn-sm" onclick="ubahDaerah({{ $data->id }})">Change</button>
                                </div>
                                <form id="formDaerah{{ $data->id }}" action="{{ route('dpd.updateDaerah', $data->id) }}" method="POST" class="d-none">
                                    @csrf
                                    @method('PATCH')
                                    <div class="d-flex gap-1">
                                        <select name="wilayah_dpd_id" id="wilayah_dpd_id" class="form-select form-select-sm w-50">
                                                <option value=null></option>
                                            @foreach ($daerah as $item)
                                                <option {{ ( $item->id == $data->wilayah_dpd_id) ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->nama }}</option>
                                            @endforeach
                                        </select>
                                        <button type="button" class="btn btn-secondary btn-sm" onclick="batalUbahDaerah({{ $data->id }})">Batal</button>
                                        <button type="submit" class="btn btn-success btn-sm">Simpan</button>
                                    </div>
                                </form>
                            </td>
                            ...
                            <td class="d-flex gap-1">
                                <form action="{{ route('dpd.destroy', $data->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button onclick="return confirm('Yakin Untuk Menghapus Dari Dpd?')" class="btn btn-danger btn-sm">Hapus</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
...
@section('script')
    <script>
        ...
        function ubahDaerah(params) {
            console.log('formDaerah' +params);
            var form = document.getElementById('formDaerah' +params);
            var inputReadonlyDaerah = document.getElementById('inputReadonlyDaerah' +params);
            form.classList.remove("d-none");
            inputReadonlyDaerah.classList.add("d-none");
        }
        function batalUbahDaerah(params) {
            var form = document.getElementById('formDaerah' +params);
            var inputReadonlyDaerah = document.getElementById('inputReadonlyDaerah' +params);
            form.classList.add("d-none");
            inputReadonlyDaerah.classList.remove("d-none");
        }
